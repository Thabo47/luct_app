...
export default function ReportFormFields({ selectedClass = null }) {
  ...
  // Auto-fetch total registered students when courseCode changes
  useEffect(() => {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      // Save total students for this course so it auto-fills next time
      if (totalStudents) {
      ...
      // Reset
      setTopic(''); setOutcomes(''); setRecommendations('');
      setStudentsPresent(''); setDateOfLecture('');
    } catch (e) {
...
const styles = StyleSheet.create({
  ...
  input:        { backgroundColor: '#1e293b', color: '#f1f5f9', borderRadius: 10,
  ...
  weekBtn:      { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8,
  ...
  btn:          { backgroundColor: '#6366f1', borderRadius: 12, paddingVertical: 15,
